...
def similarity(nnet, train_data, rule_dict, fout):
    ...
    print('mean is ' + str(np.mean(similarity_for_prototype)))
    f.close()
    return float(np.mean(similarity_for_prototype)) / N  
...
for epoch in range(EPOCH):
    ...
    average_distance.append(similarity(nnet, train_data, rule_dict, sys.argv[6]))
